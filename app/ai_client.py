...
【输出格式】如果你生成邮件草稿，必须只生成一封完整邮件，并将这一封草稿包裹在 [DRAFT_START] 和 [DRAFT_END] 之间。
系统会自动提取标签内的内容放入用户的草稿箱。
每次回复最多只能出现一对 [DRAFT_START] / [DRAFT_END] 标签。
不要给两个版本、两个备选草稿、先后两篇邮件，或在标签外再写一封完整邮件。
除了标签内的内容，你也可以在外部用一两句自然且同理心的语言辅导和安抚他们，但标签外不得包含称呼、正文、落款等完整邮件结构。
...
2. Reduce interaction overhead — provide one direct correction or one revised draft.
...
_DRAFT_BLOCK_RE = re.compile(r"\[DRAFT_START\][\s\S]*?\[DRAFT_END\]", re.IGNORECASE)


def _keep_single_draft_block(text: str) -> str:
    """Prevent accidental multiple draft blocks from reaching the UI."""
    blocks = list(_DRAFT_BLOCK_RE.finditer(text or ""))
    if len(blocks) <= 1:
        return text
    first = blocks[0]
    prefix = text[:first.start()].strip()
    draft = first.group(0).strip()
    return f"{prefix}\n\n{draft}".strip() if prefix else draft
...
"""
        ...
        return _keep_single_draft_block(data["choices"][0]["message"]["content"])
